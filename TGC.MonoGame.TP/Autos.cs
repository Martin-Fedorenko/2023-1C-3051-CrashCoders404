  ...
  public class Autos
  {
      ...
      for (var index = 0; index < cantidadEnemigos; index++)
      {
        if(autosDestruidos.Contains(index))
        {
          if(index < 5)
            ...
            aabbIA[index] = new BoundingBox(AutoDeCombateBoxAABB.Min + posicionEspera - coreccionAlturaAutoCombate, AutoDeCombateBoxAABB.Max + posicionEspera - coreccionAlturaAutoCombate);
    ...
    public void dibujarAutos(Matrix view, Matrix projection, Effect effect, String tecnica, BoundingFrustum boundingFrustum)
    {
      ...
      for (int index = 0; index < cantidadEnemigos; index++)
      {
          ...
          if(boundingFrustum.Intersects(aabbIA[index]) || dissolveActivado[index])
          {
